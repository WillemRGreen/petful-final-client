...
export default class DogAdoption extends Component {
  ...
  intervalForDemo() {
    let adding = false;
    let newUsers = ['Willem', 'test1', 'test2', 'test3', 'test4'];
  ...
  render() {
    ...
    return (
      <div className='adoption'>
        ...
        {this.props.dogs.length <= 0 ? <h2> No pets available </h2> : null}
        <div className='list'>{dogPage}</div>
        <div className='queue'>{dogListMap}</div>{' '}
        {this.props.dogList.length <= 0 ? (
          <h2> Congrats on your adoption! </h2>
        ) : null}{' '}
      </div>
    );
  }
